...
Codex 앱의 브라우저 패널이나 일반 브라우저에서 [상태 화면](http://127.0.0.1:8765/status)을 열면 전체 Codex 작업의 최근 **실제 응답 모델**과 **요청한 effort**가 3초마다 갱신됩니다. 앱의 effort 선택은 세션 기본값으로 남습니다. 보고서의 `byEffort`도 요청한 턴별 값입니다.
